...
- **后续建议**：~~可加一条轻量静态检查（lint 规则）自动拦截"组件内 fetch""空 catch"~~ → **已落地**：`full` 级别内置 `eslint.config.mjs`，把"组件内 fetch / 空 catch / 裸 console"纳入 `npm run check` 机器兜底（详见 [`03` 第 6.5 节](./03-测试体系设计.md)）。
